    ...
    */
    ...
    public function fetchProgramChartDetails(Request $request) 
    {
        ...
        if($request->filled('program_id'))
        {
            ...
            $programs   = $query->with('template')->with('category')->with(['subcategory.exercisesub'=>function($que) use($request){
            ...
            $programData    = $query2->with('category','subcategory.exercisesub')->whereIn('category_id',$category_ids)->get();
            ...
            if($request->main_template_id) {
                ...
                $programs = $query->with('template')->with('category')->with(['subcategory.exercisesub'=>function($que) use($request,$assessment_cat_id){
                ...
                $programData  = $first_portion_query->with('category','subcategory.exercisesub')->whereIn('category_id',$category_ids)->get();
